...
def test_runs_param_files_sequentially_in_sorted_order(
    ...
    assert order_path.read_text().splitlines() == [
        "swmf:first",
        "post:first:res/run001_20200101_000000_20200101_010000",
        "swmf:second",
        "post:second:res/run002_20200102_000000_20200102_010000",
    ]
    assert results == [
        run_directory / "res/run001_20200101_000000_20200101_010000",
        run_directory / "res/run002_20200102_000000_20200102_010000",
    ]
...
def test_rejects_existing_result_before_starting_swmf(
    tmp_path: Path, monkeypatch
) -> None:
    input_directory = tmp_path / "inputs"
    input_directory.mkdir()
    input_name = "PARAM_20200101_000000_20200101_010000.in"
    (input_directory / input_name).write_text("first")
    ...
    existing_result = run_directory / "res/run001_20200101_000000_20200101_010000"
    existing_result.mkdir(parents=True)
    namespace = runpy.run_path(str(SCRIPT))

    with pytest.raises(FileExistsError, match=existing_result.name):
        namespace["run_param_files"](input_directory, run_directory=run_directory)
